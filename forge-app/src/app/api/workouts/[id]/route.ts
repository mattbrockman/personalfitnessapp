...
export async function GET(
  ...
  try {
    ...
    // Combine exercises with their sets (from normalized tables)
    ...
    // Use JSONB exercises if no normalized exercises exist (AI-created workouts)
    const workoutData = workout as any
    let finalExercises = exercisesWithSets

    if (exercisesWithSets.length === 0 && workoutData.exercises && Array.isArray(workoutData.exercises)) {
      // Convert JSONB format to expected format for lifting tracker
      finalExercises = workoutData.exercises.map((ex: any, idx: number) => ({
        id: `jsonb-${idx}`,
        exercise_id: null,
        ...
        sets: [], // Will be generated by lifting tracker based on ex.sets count
        _jsonb_sets: ex.sets, // Pass original set count
        _jsonb_reps_min: ex.reps_min,
        _jsonb_reps_max: ex.reps_max,
      }))
    }
